...
function renderOrderList() {
    ...
    // 模拟模式：读取本地记录
    const orders = (window.mockOrderHistory || []).slice().sort(function(a, b) {
...
async function renderRealOrderList(listEl, emptyEl, t, lang) {
    ...
    const orders = res.extra.orders.slice().sort(function(a, b) {
        return (new Date(b.create_time).getTime() || 0) - (new Date(a.create_time).getTime() || 0);
    });
    if (orders.length === 0) {
        if (listEl) listEl.style.display = 'none';
        if (emptyEl) { emptyEl.style.display = 'block'; emptyEl.textContent = t.orderQueryEmpty; }

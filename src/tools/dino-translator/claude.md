...
| 当前版本 | v1.2.3（已部署 `/Applications/恐龙岛翻译器.app`；3 项目标语言+源语言自动识别；默认引擎=阿里云跳板 Ollama，分发零 key） |

## 功能

- 置顶窗口，中文 / 越南语 / 英文互译
- 翻译目标语言仅 3 项（→中文 默认 / →越南语 / →英文），源语言由模型自动识别；「⇄ 换方向」循环切换目标语言（v1.2.3：原 6 方向 + 源语言声明模式废弃）
- 术语表两级合并：基础表 `glossary.json`（脚本生成，只读） + 自定义表 `custom_glossary.json`（GUI 管理，同 zh 条目自定义优先）；管理窗口支持双击编辑/新增/删除，删除基础条目=停用（空覆盖）
- 🔁 回译验证：启发式检测原文语言（汉字→中文/越南语特征字符→越南语/其他→英文），把译文翻回原文语言核对语义；回译结果区默认折叠，回译后自动展开，清空时收起

...
| 当前版本 | v1.2.4（已部署 `/Applications/恐龙岛翻译器.app`；方向=源语言表达 4 项；默认引擎=阿里云跳板 Ollama，分发零 key） |

## 功能

- 置顶窗口，中文 / 越南语 / 英文互译
- 翻译方向选项以**源语言**表达，共 4 项：`越南语`/`英语`（外语固定译中文，默认越南语）+ `中文→越南语`/`中文→英语`（中文译出）；「⇄ 换方向」镜像互换（越南语↔中文→越南语，英语↔中文→英语）（v1.2.4）
...
| 2026-09-10 | v1.2.4：方向语义改为按**源语言**表达（用户澄清）：4 项 v2z/e2z/z2v/z2e，⇄ 镜像互换，prompt 加源语言消歧，术语替换仅 z2v 生效，回译用镜像方向；废弃 detect_target |

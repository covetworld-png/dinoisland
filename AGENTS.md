...
│   │   │   └── product/    # 产品页
│   │   └── components/     # 官网通用组件
│   ├── campaign/           # 活动页（短期、临时、营销）
│   │   ├── assets/         # 活动页专属资源
│   │   ├── pages/          # 活动页（按活动主题命名）
│   │   │   └── invite-activity/    # 邀请活动模板
│   │   └── components/     # 活动页通用组件
│   └── common/             # 官网+活动页共用资源
├── public/                 # 公共静态资源（favicon、robots.txt等）
├── versions/               # 版本备份目录
└── projects/
    └── {编号}-{名称}/
        ├── claude.md       # 必须登记前端输出
        └── frontend/       # 项目专属页面（按需创建）
```
...
| 类型 | 位置 | 复用范围 | 触发创建条件 |
|------|------|----------|--------------|
| 官网页 | `src/official/pages/` | 所有涉及官网改造的项目 | 提示词/文档描述官网改造 |
| 活动页 | `src/campaign/pages/` | 所有同类活动项目 | 提示词/文档描述活动页面 |
| 项目专属页 | `projects/{编号}/frontend/` | 仅本项目 | 提示词明确为项目专属功能 |

### 8.3 版本管理

每次修改 `src/` 下文件时：

1. **备份原版本** → `versions/{原相对路径}/{版本号}/`
2. **递增版本号** → `v{major}.{minor}.{patch}`
3. **登记变更** → 更新项目 `claude.md` 的「版本历史」
...
临时计算数据、生成图表存放位置，禁止散落在根目录：

```
data/
├── charts/                 # 生成的图表文件（png/svg等）
└── exports/                # 数据导出文件（csv/json/zip等）
```

**规则**:
- 图表生成后必须移至 `data/charts/`
- 数据导出文件必须移至 `data/exports/`
- 根目录下禁止存放 `.png`, `.csv`, `.json` 等数据文件

### 8.6 项目登记（claude.md）

所有涉及前端的项目必须在 `claude.md` 中登记：

```markdown
## 前端输出清单
| 页面 | 位置类型 | 物理路径 | 版本 | 状态 |
|------|----------|----------|------|------|
| 邀请仪表盘 | 活动共享 | `src/campaign/pages/invite-activity/index.html` | v1.0.0 | 完成 |

## 版本历史
| 版本 | 日期 | 项目 | 变更说明 |
|------|------|------|----------|
| v1.0.0 | 2026-03-14 | 001-01-referral | 初始版本 |
```

...
| 操作 | 效果 |
|------|------|
| **粘贴 H2_1234** | 自动拆分到两个框：口令=`H2`，身份码=`1234` |
| **口令输入满2位** | 自动跳转到身份码输入框 |
| **身份码按回车** | 触发解析生成链接 |
| **身份码为空+Backspace** | 回退到口令框并删除最后一个字符 |
| **仅输入口令** | 身份码留空，生成兜底链接（无身份码） |

#### 功能列表

| 功能 | 说明 |
|------|------|
| **口令解析** | 双输入框设计，支持粘贴自动拆分或手动输入 |
| **手动选择** | 下拉选择主播和渠道，默认 Unknown |
| **链接生成** | 生成 `?code=口令_身份码` 或 `?code=口令` 格式的注册链接 |
| **话术模板** | 双语话术库（越南语/中文参考），左右并列显示 |
| **历史记录** | 记录生成历史，支持复制链接、下载/导入 |
